...
- Ruta: `/average`
- Método: `POST`
- Descripción: Recibe una lista de números y devuelve el promedio de sus elementos.
- Entrada: `{"numbers": [lista de números]}`
- Salida: `{"average": promedio}`

7. ### Ordenamiento Rápido (Quick Sort)
- Ruta: `/quick-sort`
- Método: `POST`
- Descripción: Recibe una lista de números y devuelve la lista ordenada utilizando el algoritmo de Quick Sort.
- Entrada: `{"numbers": [lista de números]}`
- Salida: `{"numbers": [lista de números ordenada]}`

### Paginación en Filtro de Pares
El endpoint `/filter-even` ahora soporta paginación mediante los parámetros de consulta `page` y `size`:
...
```bash
curl -X 'POST' \
  'http://localhost:8000/filter-even?token=<TOKEN>&page=2&size=2' \
  -H 'Content-Type: application/json' \
  -d '{
  "numbers": [5, 3, 8, 6, 1, 9, 2, 4]
}'
```
Respuesta:
```json
...
### Ejemplo de uso de los nuevos endpoints
#### Promedio
```bash
curl -X 'POST' \
  'http://localhost:8000/average?token=<TOKEN>' \
  -H 'Content-Type: application/json' \
  -d '{
  "numbers": [5, 3, 8, 6, 1, 9]
}'
...
```bash
curl -X 'POST' \
  'http://localhost:8000/quick-sort?token=<TOKEN>' \
  -H 'Content-Type: application/json' \
  -d '{
  "numbers": [3, 2, 1, 5, 4]
}'
